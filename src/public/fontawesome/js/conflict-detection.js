...
(function (global, factory) {
  ...
  function report (_ref) {
    ...
    if (timeOutCount > 0) {
      console.info("%cLeftovers--we timed out before collecting temp results for ".concat(timeOutCount == 1 ? 'this' : 'these', ":"), 'color: blue; font-size: large');
      var _data2 = {};
  ...
  var nodeUnderTestId = 'fa-kits-node-under-temp';
  ...
  function detectWebfontConflicts() {
    ...
    var styleTags = Array.from(DOCUMENT.getElementsByTagName('style')).filter(function (t) {
      if (t.hasAttribute(detectionIgnoreAttr)) {
        return false;
      } // If the browser has loaded the FA5 CSS, let's not temp that <style> element.
    ...
    function runDiag(scriptOrLinkTag, md5) {
      ...
      var testIconId = 'fa-temp-icon-' + md5;
  ...
  function conflictDetection() {
    ...
    // Making it configurable allows us to temp the scenario where this timeout is exceeded.
    ...
    if (testCount === 0) {
      ...
      console.info("\t%c".concat(timeoutAttr, "%c: milliseconds to wait for each temp before deciding whether it's a conflict."), 'font-weight: bold;', 'font-size: normal;');
      console.info("\t%c".concat(resultsCollectionMaxWaitAttr, "%c: milliseconds to wait for the browser to accumulate temp results before giving up."), 'font-weight: bold;', 'font-size: normal;');
      pollUntil({
